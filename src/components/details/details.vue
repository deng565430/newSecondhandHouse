<template>
  <div class="look-project">
    ...
    <scroll ref="scroll" class="list" :beforeScroll="true" :data="msg">
      <div class="context">
        ...
        <div class="middle">
          ...
          <div class="middle-tedian">
            <p>特点：
              <span :key="item" v-for="item in features">{{item}}</span>
            </p>
          </div>
          <div class="middle-m">
            <p>单价：<span :class="price !== '(未填)' ? 'store' : ''">{{price + '万'}}</span></p>
    ...
    <router-link :to="operate === '1' ? '/addkeyuan' : '/addfangyuan'" ref="addImg" class="add-img" @click="addKeyuan">
    ...
    _getRoomDetails () {
      getRoomDetails({phone: this.phone, name: this.name}).then(res => {
        ...
        if (res.data.code === 0) {
          ...
          this.type = data.type
          this.huxing = data.huxing
          this.floor = data.floor
          this.decoration = data.decoration
          this.face = data.face
          this.roomAge = data.roomAge
